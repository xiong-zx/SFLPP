...
from core.utils import setup_directories, load_gurobi_params, apply_gurobi_defaults
...
class SweepSettings:
    ...
    favor_region_demand_factor: float = (
        0.3  # scale demand in favored region (b_i & base_demand)
    )
...
def region_from_coord(
    xy: tuple[float, float], boundary: float, use_y: bool = False
) -> str:
...
def build_bar_c(
    ...
    if not coords_c or not coords_f:
        raise ValueError(
            "Instance must have customer/facility coordinates for regional tariffs."
        )

    # Precompute region tags
    region_c = {
        i: region_from_coord(coords_c[i], boundary, use_y=use_y_boundary) for i in I
    }
    region_f = {
        j: region_from_coord(coords_f[j], boundary, use_y=use_y_boundary) for j in J
    }
...
def main():
    ...
    settings_out = (
        RESULTS_DIR
        / f"{settings.config_name}_ins{settings.instance_idx}_s{settings.n_scenarios}_regional_settings.json"
    )
    ...
    boundary_val = (
        settings.y_boundary if settings.use_y_boundary else settings.x_boundary
    )
    region_c = {
        i: region_from_coord(coords_c[i], boundary_val, use_y=settings.use_y_boundary)
        for i in inst.I
    }
    region_f = {
        j: region_from_coord(coords_f[j], boundary_val, use_y=settings.use_y_boundary)
        for j in inst.J
    }
    ...
    if settings.favor_region:
        ...
        for j in inst.J:
            factor = (
                settings.favor_region_factor
                if region_f[j] == settings.favor_region
                else settings.other_region_factor
            )
        ...
        global_mean_dist = (
            sum(avg_dist_per_fac.values()) / len(avg_dist_per_fac)
            if avg_dist_per_fac
            else 1.0
        )
        eps = 1e-6
    ...
    for tau in settings.tau_levels:
        ...
        model, vars_dict = build_extensive_form_model(new_ext, alpha=0.1)
        ...
        sol = {
            ...
            "q": {
                (i, j, w): vars_dict["q"][i, j, w].X if has_sol else None
                for i in new_ext.I
                for j in new_ext.J
                for w in new_ext.W
            },
            "s": {
                (i, w): vars_dict["s"][i, w].X if has_sol else None
                for i in new_ext.I
                for w in new_ext.W
            },
        }
    ...
    for item, ext in zip(sols, exts):
        ...
        if not sol.get("has_solution"):
            print(
                f"[solve] tau={tau} status={sol['status']} obj={sol['objective']} (no solution)"
            )
            continue
        ...
        flows = {
            (i, j): sol["q"].get((i, j, scen_idx), 0.0) for i in ext.I for j in ext.J
        }
        ...
        for i in ext.I:
            for j in ext.J:
                ...
                tariffs[(i, j)] = (
                    (scen.bar_c[(i, j)] - base_c) / base_c if base_c != 0 else 0.0
                )

        solution_dict = {
            "opened_facilities": [
                j for j, v in sol["x"].items() if (v is not None and v > 0.5)
            ],
            "flows": flows,
            "prices": sol["p"],
        ...
        plot_path = (
            PLOTS_DIR
            / f"{settings.config_name}_ins{settings.instance_idx}_s{settings.n_scenarios}_tau{str(tau).replace('.', 'p')}_regional.png"
        )
        visualize_network(
            ext.instance,
            solution_dict,
